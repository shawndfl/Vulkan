...
void Texture::initialize(const std::string& filename) {
    ...
    if (!pixels) {
        LOGE("failed to load texture image! " << filename);
        return;
    }
    ...
    // create a image buffer in video memory
    ...
    if (vkCreateImage(app.getDevice(), &imageInfo, nullptr, &textureImage) != VK_SUCCESS) {
        throw std::runtime_error("failed to create image!");
    }

    // get memory requirements for this image
    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(app.getDevice(), textureImage, &memRequirements);
    ...
    if (vkAllocateMemory(app.getDevice(), &allocInfo, nullptr, &textureImageMemory) != VK_SUCCESS) {
...
VkImageView Texture::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
    ...
    if (vkCreateImageView(app.getDevice(), &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
        LOGE("Error creating image view device");
    }
